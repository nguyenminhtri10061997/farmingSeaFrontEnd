...
export default React.memo(forwardRef((props, ref) => {
  ...
  const handleOk = () => {
    if (state.selectedRow?._id) {
      confirm({
        ...
        content: 'Bạn có chắc muốn duyệt hoàn thành phiếu nhập',
        onOk: () => {
  ...
  const handleClickCancel = () => {
    confirm({
      ...
      icon: <ExclamationCircleOutlined />,
      content: 'Bạn có chắc muốn hủy phiếu nhập',
      onOk: () => {
        return new Promise((resolve) => {
          cancel().then(() => {
            resolve()
  ...
  return (
    <Modal
      title={state.selectedRow?.code || 'Biên bản nhập kho mới'}
    ...
    >
      ...
      >
        <Steps current={state.currentStep}>
          ...
          <Step status={state.selectedRow?.state === 'COMPLETED' ? 'finish' : undefined} title='Hoàn thành nhập hàng' />
        </Steps>
